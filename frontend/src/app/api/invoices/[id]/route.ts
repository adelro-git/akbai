...
export async function PATCH(
  ...
  if (data.items) {
    // E2 fix: round each line (quantity may be fractional, e.g. hours) so stored
    // centavos stay integer; sum the rounded lines for an integer subtotal.
    ...
    // E3 fix: reject a discount larger than the subtotal (would produce a
    // negative total) with a clear conversational Filipino error.
