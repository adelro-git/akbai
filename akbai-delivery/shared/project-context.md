...
### Current Phase
> Current: Phase 0B — Native Mobile Pivot — **Sprint 18 (Pre-Launch Feature Readiness) CLOSED 🟢 GREEN on agent-doable items 2026-05-29; public-release gate pending Sprint 19 Anton wave.** On `feat/18-prelaunch-readiness` (PM-orchestrated agent team, 2 batches + review-fix pass). Gap G7 (Pre-Launch Feature Readiness Gate) is code-side GREEN with the traffic-light report at `akbai-delivery/shared/sprint-18-prelaunch-gate.md`; G5 (store assets) drafts done; G6 (Kai character) usage doc + fallback documented. **Two pre-flight findings reshaped the sprint:** Sprint 17 code (PR #38) was still unmerged at session start (main was at Sprint 16) — merged first (PR #38 code + #39 retro); and an audit found Build 5 weekly/monthly reconciliation was claimed done in Sprint 10 but had never been merged to `main` — built for real this sprint. **Shipped:** migration 023 (9 subscriptions lifecycle columns — fixes a latent insert-time schema-drift crash; RLS unchanged), Build 5 reconciliation (`lib/reconciliation/` + weekly/monthly API + dashboard cards + missing-day nudges), trial-countdown banner + day-7 paywall, circuit-breaker→Sentry alert, push 7/3/1-day cron scheduler (`/api/cron/deadline-notifications` + `vercel.json` crons + `CRON_SECRET`), offline receipt scan queue, CSV export with OWASP formula-injection guard, tax-year 2027 rollover test (+ Manila-tz bug fix in `lib/deadlines/generate.ts`), reviewer/guest demo mode (triple-gated fail-closed `/api/demo-login`, off by default in prod), legal DRAFTS (`/privacy` + `/terms` in-app pages + `npc-data-classification.md`), store-listing copy DRAFT, Kai character usage doc (13 surfaces, ≥5 gate PASS), enye/special-char rendering audit. Security audit (review-security): **NO BLOCKERS** (8 findings — CSV formula injection H1, offline-queue poisoning C1/C3, trial-banner C2, replay dedup C4, OCR-error retriable C5, breaker cap label C6, sign-out image clear M1 — all fixed in-sprint). **1716/1716 tests passing** (+161 vs Sprint 17 baseline 1555). G2/G4/G7 remain IMPLEMENTED (not RESOLVED) — they close at the Sprint 19 launch wave. **Cleanup debt tracked:** ×3 bearer-compare dup, forked offline queue, duplicated recon date helpers, sequential `/api/dashboard` reads. **Sprint 19 (final pivot sprint, Anton-heavy):** Apple/Google enrollment, real RevenueCat creds + sandbox SKUs + curl smoke, iOS icons/screenshots/legal hosting, real Kai art swap, on-device Pixel 5 smoke, Sentry symbolication, store submission — full checklist at `akbai-delivery/shared/sprint-19-launch-package.md`. **Previous milestone:** Sprint 17 (RevenueCat IAP Integration) CLOSED 🟡 YELLOW on `feat/17-revenuecat-iap` (2026-05-27, PR #38). Gap G2 (IAP webhook idempotency) IMPLEMENTED at architecture + plumbing level — full close-out at Sprint 19 enrollment wave when real RevenueCat dashboard + sandbox SKUs + on-device testing land. YELLOW signal is bundle-size-only (`.aab` 23.27 MB; 0.27 MB over architect's <23 MB flexible target; 22% under 30 MB Pre-Launch Gate ceiling); all other criteria GREEN. **Shipped this sprint:** (1) migration 022 — `revenuecat_events` table with `event_id TEXT PRIMARY KEY` as the load-bearing Gap G2 idempotency invariant + service-role-only RLS + partial indexes; `set_user_tier_v2()` SECURITY DEFINER RPC preserves `started_at` on renewals via `p_reset_started_at` flag (v1 untouched for legacy Xendit callsites); `subscriptions_tier_check` CHECK constraint locks enum to `('free','starter','pro','business','scale')`. (2) `@revenuecat/purchases-capacitor@13.1.2` integration in `lib/iap/{configure,entitlements,purchase,schemas,server-entitlements}.ts` modules — all native-only behind `Capacitor.isNativePlatform()` gate; configure idempotent (mirrors `initSentryCapacitor` precedent), entitlements tier-priority pro > starter > free, purchase error-mapped to conversational Filipino keys with `userCancelled` silent return + Sentry tags per error type. (3) Webhook handler `/api/webhooks/revenuecat/route.ts` mirroring Xendit shape: constant-time `Authorization: Bearer` signature compare against `REVENUECAT_WEBHOOK_AUTH`, idempotent `INSERT … ON CONFLICT (event_id) DO NOTHING` with 23505 → 200 OK `{ deduped: true }` skip downstream, environment guard drops SANDBOX in production, dispatch to handler calling `set_user_tier_v2` with correct `p_reset_started_at` per 11 event types, `processed_at` marked only post-success. (4) Paywall UX: `<PaywallModal />` + `<PaywallTierCard />` × 3 (Starter ₱299 / Pro Monthly ₱499 / Pro Annual ₱4,999, "Pinaka-sulit" badge on annual) + `<RestorePurchasesLink />` per Apple Guideline 3.1.1, source-keyed titles for 6 surfaces (chat / morning_briefing / weekly_story / reply_drafter / scan_limit / manual), 5 surface wirings routed (chat 429 catch, free-tier banner → onUpgrade, upgrade-prompt default → modal, morning-briefing-card reason='tier_required' → modal, /profile new Subscription section). (5) `SubscriptionTierEnum` extended to `['free', 'starter', 'pro', 'business', 'scale']`. (6) gradle.properties persist-fix `systemProp.javax.net.ssl.trustStoreType=Windows-ROOT` (Sprint 17 DRIFT — JVM-layer analog of Sprint 16 DRIFT-4 npm `--use-system-ca` recipe). **1555/1555 tests passing** (+128 vs Sprint 16). Security pass CLEAN across 4 audit surfaces (idempotency end-to-end, signature verify, environment guard, subscriptions write-path zero direct `.tier` writes). UX B+ + marketing A- voice grades; 10 review items applied in single follow-up commit (2 a11y blockers + 5 copy fixes + 3 marketing HIGH rewrites + CTA verb differentiation: Starter "Kunin", Pro tiers "Mag-subscribe"). New architect reference at `akbai-delivery/skills/solutions-architect/references/sprint-17-revenuecat-pattern.md` (1200 lines, 13 sections + 5 Open Questions held at PR review). On-device Pixel 5 smoke deferred to Sprint 19 Phase A. **Next sprint:** Sprint 18 — Pre-Launch Feature Readiness Gate review + asset readiness (G5 App Store assets, G6 Kai character integration, G7 Pre-Launch Gate). Fifth of six pure-dev sprints. **Previous milestones:** Sprint 16 (Native Surface Polish) closed GREEN via PR #35 (Gap G4 IMPLEMENTED, 5 native Capacitor plugins, .aab 20.75 MB); Sprint 15 (Capacitor Conversion) closed GREEN via PR #33 (Gap G1 resolved); Sprint 14 (Foundations + Spike) closed GREEN via PR #31 (ADR-019 Accepted Green); Sprint 13 (Frontend Redesign Phase 8-9 close-out) merged via PR #30. Full pivot plan: `C:\Users\Anton del Rosario\.claude\plans\lets-review-our-approach-tidy-harp.md`. Kai character evolution via **Gemini image generation** (prompt library: `akbai-delivery/skills/ux-designer/references/kai-gemini-prompts.md`).
...
- **Sprint 17 — RevenueCat IAP Integration** (2026-05-27, CLOSED 🟡 YELLOW on `feat/17-revenuecat-iap`, PR #38):
  - **Architect's canonical pattern doc** at `akbai-delivery/skills/solutions-architect/references/sprint-17-revenuecat-pattern.md` (1200 lines, 13 sections) — 5 Open Questions held at PR review (sandbox/prod env split, trial expiration timing, lifetime+pro coexistence state machine, webhook IP allowlist, set_user_tier_v2 RPC shape). Sprint 17 prevented Sprint 16 DRIFT-7 (PM/architect batch shape mismatch) by being explicit per §8 about which files land in which engineer batch.
  - **Migration 022** (`022_revenuecat_events.sql`): NEW `revenuecat_events` table with `event_id TEXT PRIMARY KEY` (load-bearing Gap G2 idempotency invariant), `event_type/app_user_id/event_at/payload JSONB/processed_at/deleted_at` columns, service-role-only RLS (internal audit log, no user SELECT policy), partial indexes on `(app_user_id, event_at DESC)` and on unprocessed events. `set_user_tier_v2()` SECURITY DEFINER RPC (5-arg: `p_user_id UUID, p_tier TEXT, p_expires_at TIMESTAMPTZ, p_revenuecat_app_user_id TEXT, p_reset_started_at BOOLEAN`) preserves `started_at` on renewals via the new `p_reset_started_at` flag — fixes v1's unconditional reset bug (v1 untouched for legacy Xendit callsites). `subscriptions_tier_check` CHECK constraint locks `tier` to `('free','starter','pro','business','scale')`. Idempotent DO-block guard for re-runnability across PG 14/15/16. Inline smoke-verification SQL footer for paste-into-Studio.
  - **`@revenuecat/purchases-capacitor@13.1.2`** integration:
    - `lib/iap/configure.ts` — `initRevenueCat(supabaseUserId)` module-singleton idempotent (mirrors `initSentryCapacitor` precedent), `Capacitor.isNativePlatform()` web no-op, defers if `supabaseUserId` is null, platform-specific API key (`NEXT_PUBLIC_REVENUECAT_APPLE_API_KEY` / `_GOOGLE_API_KEY`), Sentry instrumentation on catch.
    - `lib/iap/entitlements.ts` — `getEntitlements()` wraps `Purchases.getCustomerInfo()`, tier priority `pro > starter > free`, entitlement IDs `pro_unlimited` + `starter_lifetime`, failure fallback to `{ tier: 'free', isNative: true }` with Sentry capture.
    - `lib/iap/purchase.ts` — `purchasePackage(productId)` + `restorePurchases()` wrappers, `PRODUCT_TO_TIER` map (`akbai_pro_monthly`/`akbai_pro_annual` → pro; `akbai_starter_lifetime` → starter), `userCancelled` → silent return, error-code dual-form normaliser (numeric `'10'` + name `'NETWORK_ERROR'` both routed — Sprint 17 DRIFT vs architect spec) mapping to 8 conversational Filipino `iap.error.*` keys.
    - `lib/iap/schemas.ts` — `RevenueCatEventSchema` Zod envelope with `.passthrough()` for forward-compat. Defined here even though consumed by webhook in batch 2 (architect §8 explicit).
    - `lib/iap/server-entitlements.ts` — server-only `fetchEntitlementsFromRevenueCat(appUserId)` calls RevenueCat REST `GET /v1/subscribers/{app_user_id}` with `REVENUECAT_REST_API_KEY` Bearer token; pure fallback to `{ tier: 'free' }` if key missing (Sprint 19 plugs real key). Resolves Open Q 3a lifetime+pro coexistence at the entitlement-list level.
    - `(app)/layout.tsx` — new 4th `useEffect` calls `initRevenueCat(user?.id)` after `supabase.auth.getUser()` resolves; idempotent guard inside `configure.ts` makes re-fires safe.
  - **Webhook handler** `/api/webhooks/revenuecat/route.ts` mirrors Xendit handler shape (Sprint 4 precedent at lines 40-228):
    - `verifyWebhookSignature()` constant-time compare of `Authorization: Bearer <secret>` against `REVENUECAT_WEBHOOK_AUTH`; fail-closed on missing env var or malformed prefix; signature failure ALWAYS returns 200 OK (no retry storm — RevenueCat retries on non-2xx).
    - Body parse + Zod envelope validation; both failures return 200 OK silent.
    - Idempotent insert into `revenuecat_events` via Supabase JS — Postgres `23505` duplicate-key path returns 200 OK with `{ deduped: true }` and skips downstream processing. Single source of dedup: `event_id` PK at DB layer.
    - Environment guard at handler: `process.env.NODE_ENV === 'production' && event.environment !== 'PRODUCTION'` drops SANDBOX events in production (Sprint 19 sandbox testing won't mutate prod tier rows). `SANDBOX` events in NODE_ENV='development' proceed; `PRODUCTION` events always proceed.
    - Event dispatch: `INITIAL_PURCHASE` + `NON_RENEWING_PURCHASE` → `set_user_tier_v2` with `p_reset_started_at=true`; `RENEWAL` + `PRODUCT_CHANGE` + `CANCELLATION` + `EXPIRATION` + `BILLING_ISSUE` + `UNCANCELLATION` → `p_reset_started_at=false`; `SUBSCRIBER_ALIAS` + `TRANSFER` + `TEST` → log-only no tier write.
    - Server-role client (`createServiceClient()`) for all DB writes (RLS bypass per Sprint 3 architecture); ZERO direct `subscriptions.tier` writes (security audit grep-verified 0 matches in route.ts at runtime; 1 docstring self-reference acceptable).
    - `processed_at` marked only after `set_user_tier_v2` succeeds — partial failure leaves the event row re-processable on a future webhook delivery.
    - Sentry instrumentation with `tags: { source: 'revenuecat-webhook' }`.
  - **`SubscriptionTierEnum` extension** (`lib/subscriptions/types.ts`): `['free','pro','business','scale']` → `['free','starter','pro','business','scale']`. `SCAN_LIMITS` gained `starter: 100`. Zero non-test callsite changes required — all existing tier checks remain correct under the new enum.
  - **Paywall UX** (the highest-touch revenue surface this sprint):
    - `<PaywallModal />` — fixed-overlay div + role="dialog" + aria-modal + Escape-to-close keydown listener (Sprint 7+ `check-in-modal.tsx` precedent; vitest's node env blocks Radix dialog). `Capacitor.isNativePlatform()` branches: native renders 3 tier cards + restore link; web renders fallback CTA + App Store/Play Store text-placeholder badges (no purchase buttons). Source-keyed titles for 6 sources (`chat` / `morning_briefing` / `weekly_story` / `reply_drafter` / `scan_limit` / `manual`). Conditional `paywall.body.starter_only` body renders only when source ∈ {'chat', 'manual'} (non-sequitur on other sources). Error states surface via inline error strip with `useTranslations('iap.error')` (DRIFT vs architect's toast spec — codebase's `use-toast.js` is unused; build-ux can swap later).
    - `<PaywallTierCard />` — reusable, called 3x with different props. Per-card content keys (`paywall.cards.{starter,pro_monthly,pro_annual}.{title,subtitle,bullets,note,cta}`). "Pinaka-sulit" badge on annual default-highlighted card. `purchasing` prop disables CTA during in-flight purchase.
    - `<RestorePurchasesLink />` — reusable; mounted in PaywallModal footer AND `/profile` Subscription section (Apple Guideline 3.1.1 — restore link MUST be visible, not collapsed behind a "More" drawer).
    - 5 surface wirings: `chat-interface.tsx` (catch 429 with `free_tier_limit` → PaywallModal source='chat'), `free-tier-banner.tsx` (`/pricing` href → onUpgrade callback opens modal — also removed No-Line Rule violation borders), `upgrade-prompt.tsx` (self-mounts PaywallModal when no `onUpgrade` prop), `morning-briefing-card.tsx` (`reason='tier_required'` → PaywallModal source='morning_briefing'), `profile-view.tsx` (new "Plano Mo" Subscription section with manual paywall trigger + restore link). NOT WIRED (Sprint 18+): `kuwento-card.tsx` weekly_story paywall, scan-limit gate, reply_drafter (no component file exists yet).
    - 22 conversational Filipino `paywall.*` string keys + 10 `iap.*` keys (errors + success) in `messages/fil.json` + `messages/en.json`. Marketing voice review (A-) applied 3 HIGH rewrites: Starter subtitle "₱299 — isang beses lang ang bayad" (dropped "lifetime" English label), Annual subtitle "matitipid mo ang ₱990" (more aspirational verb, dropped "halos"), Starter no-chat note reframed from exclusionary to aspirational. CTA verb differentiation: Starter keeps "Kunin" (one-time purchase metaphor); Pro Monthly/Annual now "Mag-subscribe sa Pro / Pro Annual" (recurring nature).
  - **gradle.properties persist-fix** (`712c729`): `systemProp.javax.net.ssl.trustStoreType=Windows-ROOT` persisted so future builds don't need the CLI flag. JVM-layer analog of Sprint 16 DRIFT-4 npm `--use-system-ca` recipe. Sprint 17 DRIFT (new corporate-TLS surface — Maven fetch for `com.revenuecat.purchases:purchases-hybrid-common:18.7.0` hit PKIX SunCertPathBuilderException because JDK cacerts trust store doesn't contain the corporate root CA; `Windows-ROOT` delegates to OS trust store which has the corporate CA enrolled).
  - **Decision-gate metrics:** Web build PASS (49 routes; new `/api/webhooks/revenuecat` registered as dynamic ƒ). Capacitor static export `out/` = 16 MB / 304 files / 21 prerendered routes (+2 files vs Sprint 16: paywall components). `npx cap sync android` registers 7 plugins (+`@revenuecat/purchases-capacitor` over Sprint 16's 6). **`.aab` = 23.27 MB (Sprint 16: 20.75 MB; +2.52 MB; 22% under 30 MB Pre-Launch Gate; 0.27 MB OVER architect's <23 MB flexible target → YELLOW signal).** `.apk` = 29.39 MB (Sprint 16: 24.39 MB; +5.00 MB — fat debug; Play Store split-APK from AAB is user-facing payload, ~half of this).
  - **1555/1555 vitest tests passing** across 113 files (+128 vs Sprint 16 baseline 1427): configure +9, entitlements +7, purchase +19, webhook route +32, server-entitlements +14, paywall-modal/restore-link/paywall-trigger/morning-briefing-paywall +47. TypeScript: 49 errors (42 Sprint 16 baseline + 7 net-new in `lib/iap/__tests__/configure.test.ts` — same `vi.mocked` spread-arg pattern as Sprint 16's profile-test propagation; **0 NEW patterns introduced by Sprint 17 production code**).
  - **Security pass:** CLEAN across 4 audit surfaces. (1) Webhook idempotency end-to-end: `event_id TEXT PRIMARY KEY` invariant, `23505` → 200 OK `{ deduped: true }` skip, `processed_at` only set post-`set_user_tier_v2` success, ZERO direct `.from('subscriptions').update(` matches in route.ts at runtime. (2) Signature verification: constant-time XOR compare, fail-closed on missing env, missing/malformed Authorization both return 200 OK silent. (3) Environment guard: SANDBOX in prod NODE_ENV drops; SANDBOX in dev proceeds; PRODUCTION always proceeds. (4) Subscriptions write-path: all tier writes route through `set_user_tier_v2` RPC, SECURITY DEFINER, hard-coded `WHERE user_id = p_user_id AND deleted_at IS NULL` predicate. NPC-compliant: only pseudonymous `app_user_id = users.id::text` sent to RevenueCat (no email/phone/name). 1 minor note: length short-circuit in constant-time compare (inherited from Xendit handler, not new risk; flagged for retro record only).
  - **UX pass:** B+ voice grade (UX) + A- voice grade (marketing). 2 a11y blockers fixed (No-Line Rule violation in free-tier-banner, missing `aria-labelledby`/`aria-describedby` on PaywallModal). 5 copy fixes (English-SVO leaks "Learn about Pro", "for today", "or upgrade to Pro" → conversational Filipino; raw `errorKey` rendered as literal text → translated via `useTranslations('iap.error')`; profile section "Subscription" → "Plano Mo"; upgrade-prompt wrong scan count → unlimited for Pro). 4 marketing rewrites (Starter "isang beses lang" anchor, Annual "matitipid mo" verb, Starter no-chat aspirational reframe, CTA verb differentiation). All 10 items landed in single follow-up commit (`f3b7ec1`).
  - **DRIFT items** (architect spec vs reality): (1) **NEW: Corporate-TLS at JVM/Gradle layer** — Maven fetch for RevenueCat hybrid-common artifact hit PKIX exception; resolved with `Windows-ROOT` trust store flag, persisted to `gradle.properties`. JVM analog of Sprint 16 DRIFT-4. (2) RevenueCat SDK error code form: architect spec used name strings (`'NETWORK_ERROR'`); SDK exposes numeric strings (`'10'`); engineer defensively normalised both. (3) Bundle-size guard ceiling: architect proposed 22 → 23 MB but on-disk ceiling was already 30 MB (Pre-Launch Gate); engineer kept 30 MB intact (architect "raise" would have lowered the existing gate). (4) SubscriptionTierEnum cross-batch: batch 1 used local `IapTier` narrow union; batch 2 extended global enum to include `'starter'`. (5) Toast vs inline error strip (architect spec vs codebase reality — `use-toast.js` unused). (6) Modal primitive: fixed-overlay div instead of Radix dialog (vitest node env blocks Radix). (7) `/profile` structural: modified `profile-view.tsx` (actual UI) instead of `page.tsx` (thin wrapper). (8) Store badges: text placeholders pending real PNG assets (Sprint 18+). Plus 2 build-data drift findings flagged for Anton verification: `protect_feature_flags()` trigger missing from any migration file (referenced in docs as deployed Sprint 3); `lib/subscriptions/lifecycle.ts` writes 9 columns not in migration 003 (either added via Studio direct SQL or Xendit code silently broken).
  - **Velocity:** Anton ~0.5 hr (third consecutive pure-dev sprint at 4-6x under 2-3 hr budget); agent ~6 hr wall-clock (architect ~10 min, data ~2 min, 3 engineer batches ~30 min, UX + marketing reviews parallel ~2 min each, engineer follow-up ~6 min, QA ~3 min, security ~2 min, PM-owned gradle ~3 min after corporate-TLS recipe applied — ~10 min exploration for JAVA_HOME/ANDROID_HOME/cert chain before persist-fix; well under 12-15 hr budget). Wall-clock ~7 hr from sprint start to PR open (slightly heavier than Sprint 16's ~5 hr due to gradle TLS exploration; `gradle.properties` persist-fix means Sprint 18+ avoid this entirely).
  - **7 commits** on `feat/17-revenuecat-iap`: 1 architect doc, 1 migration, 3 engineer batches, 1 UX+marketing follow-up, 1 chore (gradle TLS persist-fix). PR #38 open, awaiting merge.

- **Sprint 18 — Pre-Launch Feature Readiness** (2026-05-29, CLOSED 🟢 GREEN on agent-doable items on `feat/18-prelaunch-readiness`; public-release gate pending Sprint 19):
  - **PM-orchestrated agent team** (build-data, build-engineer ×, marketing-content, review-security, + general-purpose) across 2 batches + a review-fix pass. Goal: drive every agent-doable Pre-Launch Gate item (Gap G7, plan §11) to GREEN.
  - **Two pre-flight findings:** (1) Sprint 17 code (PR #38) was still unmerged — `main` was at Sprint 16; merged Sprint 17 first (PR #38 code + #39 retro). (2) Build 5 weekly/monthly reconciliation was logged DONE in Sprint 10 but had never been merged to `main` — built for real this sprint.
  - **Migration 023** (`023_subscriptions_lifecycle_columns.sql`): 9 subscriptions lifecycle columns (`current_period_start/_end`, `scan_limit`, `scans_used_this_period`, `grace_period_end`, `grace_notifications_sent`, `payment_method`, `xendit_customer_id`, `cancelled_at`) — fixes a latent schema-drift bug where `lib/subscriptions/lifecycle.ts` wrote columns no migration defined (inserts would throw). RLS unchanged (SELECT-only). `Subscription` type gained `started_at`.
  - **Build 5 reconciliation (built for real):** `lib/reconciliation/` (aggregate + types), `/api/reconciliation/{weekly,monthly}`, weekly + monthly dashboard cards with missing-day nudges, `home.reconciliation.*` i18n (FIL+EN).
  - **Trial-countdown banner:** `lib/subscriptions/trial.ts` + `trial-countdown-banner.tsx` + `trialCountdown.*` i18n — 7-day countdown, day-7 paywall trigger; renders on trialing + expired.
  - **Resilience/scheduling:** circuit-breaker → Sentry alert on trip/warning (`captureMessage`, dedup fingerprint, correct global/user cap label); push 7/3/1-day scheduler wired via `/api/cron/deadline-notifications` + `vercel.json` crons + `CRON_SECRET` (trigger logic existed but was never invoked); Capacitor `SplashScreen` config (needs `@capacitor/splash-screen` install — Sprint 19).
  - **Receipts/exports:** offline receipt scan queue (`lib/ocr/offline-queue.ts` + scanner-flow wiring) with validate-before-save, attempt cap, dedup, retriable-error handling; CSV export (`lib/expenses/csv.ts` + button) with OWASP formula-injection guard; tax-year 2027 rollover unit test (+ fixed a real UTC bug in `lib/deadlines/generate.ts` that used runtime tz instead of Manila).
  - **Reviewer/guest mode (P0):** seeded demo account (`seed-demo-account.sql`) + triple-gated fail-closed `/api/demo-login` + gated login button. Off by default in prod.
  - **Legal + listing + Kai DRAFTS:** `/privacy` + `/terms` in-app pages (prominent DRAFT banners, pending PH lawyer per Gap A2); `npc-data-classification.md` matrix; `store-listing-copy.md` DRAFT; `kai-character-usage.md` (Kai in 13 surfaces, ≥5 gate PASS + fallback policy); enye/special-char rendering audit (7 break-words/overflow-wrap fixes).
  - **Security audit (review-security): NO BLOCKERS.** 8 findings all fixed in the review-fix pass: CSV formula injection (H1), offline-queue poisoning (C1/C3), trial-banner expired-branch reachability (C2), replay dedup (C4), non-JSON OCR error retriable (C5), breaker cap mislabel (C6), clear offline images on sign-out (M1).
