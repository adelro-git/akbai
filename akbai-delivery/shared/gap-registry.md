...
3. **Taglish Style Guide + Prompt Regression Tests** — Write formal Taglish style guide before Phase 1 build begins. Build a 20–30 case test library and run it every time the system prompt or Claude model is updated. **Knowledge prerequisites resolved 2026-03-22:** BIR Knowledge Base (`bir-knowledge-base.md`), MSME Business Knowledge (`msme-business-knowledge.md`), and Kilala Kita Onboarding Context (`kilala-kita-context.md`) are complete and wired into TAX_SCOPE. Taglish style guide (Sprint 2 Task 4) still pending — requires Anton's qualitative review.
