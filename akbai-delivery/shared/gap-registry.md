...
> Last updated: 2026-05-29 (Sprint 18 — Pre-Launch Feature Readiness: G7 code-side GREEN + gate report; G5 drafts done; G6 usage doc + fallback documented; subscriptions schema-drift fixed via migration 023; public-release items Anton-gated Sprint 19) | Prior source: Roadmap v14, Operations Playbook v7
> 40 total gaps across 7 categories. Category G (native mobile pivot) adds 7 new gaps; G1 RESOLVED (Sprint 15); G4 + G2 IMPLEMENTED (Sprint 16 + 17); G5 + G6 drafts/usage done (Sprint 18); G7 code-side GREEN (Sprint 18). G2/G4/G5/G6/G7 all close at Sprint 19 enrollment/launch wave.
...
> **Sprint 18 note (2026-05-29) — subscriptions schema-drift fixed:** Migration 023 (`023_subscriptions_lifecycle_columns.sql`) adds the 9 lifecycle columns that `lib/subscriptions/lifecycle.ts` was already writing but no migration defined (`current_period_start/_end`, `scan_limit`, `scans_used_this_period`, `grace_period_end`, `grace_notifications_sent`, `payment_method`, `xendit_customer_id`, `cancelled_at`). This was a latent insert-time crash carried since the Xendit era; flagged by build-data in Sprint 17, closed in Sprint 18. RLS unchanged (SELECT-only). `Subscription` type gained `started_at`. (Note: the separate `protect_feature_flags()` trigger-drift item flagged in Sprint 17 remains OPEN for Anton to verify in Supabase Studio — see sprint-history Sprint 17 action item #6.)
...
| G2 | IAP webhook idempotency (RevenueCat) | **CRITICAL** | ✅ IMPLEMENTED 2026-05-27 (Sprint 17) — full close-out at Sprint 19 enrollment wave | Replaces D2. Sprint 17 (PR #38) shipped: migration 022 (`revenuecat_events` table with `event_id TEXT PRIMARY KEY` = the load-bearing G2 invariant; service-role-only RLS; partial indexes for tier-history queries and unprocessed events; soft-delete invariant), `set_user_tier_v2()` SECURITY DEFINER RPC (preserves `started_at` on renewals via `p_reset_started_at` flag — fixes v1's unconditional reset; v1 untouched for legacy Xendit callsites), `subscriptions_tier_check` CHECK constraint locking enum to `('free','starter','pro','business','scale')`. `/api/webhooks/revenuecat/route.ts` mirrors Xendit handler shape: constant-time `Authorization: Bearer` signature compare against `REVENUECAT_WEBHOOK_AUTH`, body parse + Zod envelope validation, idempotent `INSERT … ON CONFLICT` via `event_id` PK (23505 duplicate-key → 200 OK `{ deduped: true }` skip downstream), environment guard drops SANDBOX events in production NODE_ENV, dispatch to handler calling `set_user_tier_v2` with correct `p_reset_started_at` per event type (`INITIAL_PURCHASE`/`NON_RENEWING_PURCHASE` = true; `RENEWAL`/`PRODUCT_CHANGE`/`CANCELLATION`/`EXPIRATION`/`BILLING_ISSUE` = false; `SUBSCRIBER_ALIAS`/`TRANSFER`/`TEST` = log-only no write), `processed_at` marked only post-success (partial failure leaves event re-processable). Server-side REST entitlement resolver (`server-entitlements.ts`) re-computes canonical tier via `GET /v1/subscribers/{app_user_id}` — never trusts single event's product_id since Starter + Pro can coexist. Client (`lib/iap/{configure,entitlements,purchase}.ts`) initialises RevenueCat once per app open behind `Capacitor.isNativePlatform()` gate; web/PWA fallback shows "open in app to purchase" CTA. Paywall (`<PaywallModal />`) shipped with 3 tier cards (Starter ₱299 / Pro Monthly ₱499 / Pro Annual ₱4,999), source-keyed titles for 6 surfaces, restore-purchases link visible per Apple Guideline 3.1.1. **Security pass CLEAN** across 4 audit surfaces: webhook idempotency end-to-end, signature verification (constant-time compare + fail-closed on missing env + always-200), environment guard, subscriptions write-path (grep confirms 0 direct `.from('subscriptions').update(` matches in webhook route; all writes through `set_user_tier_v2` RPC). NPC-compliant (only pseudonymous `app_user_id = users.id::text` sent to RevenueCat — no email/phone/name). 1555/1555 tests passing. `.aab` 23.27 MB (22% under 30 MB Pre-Launch Gate). Full close-out at Sprint 19 enrollment wave when real RevenueCat dashboard + sandbox testing + iOS Info.plist + curl smoke test against staging webhook with real shared secret land. |
...
| G4 | Apple Guideline 4.2 rejection risk (webview app) | IMPORTANT | ✅ IMPLEMENTED 2026-05-27 (Sprint 16) — full close-out at Sprint 18 Pre-Launch Gate review | Sprint 16 (PR #35) shipped 5 native plugin integrations on `feat/16-native-polish`: `@capacitor/camera@8.2.0` (native `Camera.getPhoto` on `/scan`), `@capacitor/push-notifications@8.1.1` (FCM/APNs via discriminated-union `/api/push/subscribe` Zod; deferred prompt on `/deadlines` first-view), `@aparajita/capacitor-biometric-auth@10.0.0` (substituted from architect spec `@capacitor-community/biometric-auth` which doesn't exist on npm — same API; onboarding step 6.25, `(app)/layout.tsx` app-open guard with 3-strike OTP fallback, `/profile` toggle), `@capacitor/app@8.1.0` (deep linking for `com.akbai.app://auth/callback`), `@sentry/capacitor@4.0.0` (native crash SDK alongside `@sentry/nextjs`; ProGuard mapping + `scripts/upload-symbols.{ps1,sh}` configured for Sprint 19 execution). Plus migrations 020 (push_subscriptions platform extension) + 021 (users biometric columns). Bundle impact: `.aab` 14.62 → 20.75 MB (+6.13 MB; **31% under 30 MB Pre-Launch Gate**). 1427/1427 tests passing. Security pass MINOR ISSUES only (no blockers). The ensemble produces a credible "more than a webview" signal for App Store review: biometric step 6.25 (LAContext privileged API), native camera modal, native push permission dialog, custom-scheme deep link, native crash SDK. Plan for 1 rejection cycle still (1-2 weeks) — full close-out at Sprint 18 Pre-Launch Gate review when real Firebase/APNs creds + iOS Info.plist + on-device smoke land. |
| G5 | App Store assets (icons, screenshots, listing copy, privacy policy) | IMPORTANT | Sprint 18 (drafts) → Sprint 19 (publish) | 🟡 DRAFTS DONE 2026-05-29 (Sprint 18): conversational Filipino store listing copy DRAFT (`store-listing-copy.md`), in-app `/privacy` + `/terms` pages with prominent DRAFT banners (pending PH lawyer per Gap A2), NPC RA 10173 data-classification matrix (`npc-data-classification.md`), Android app icons, reviewer/guest demo account (seeded `seed-demo-account.sql` + triple-gated fail-closed `/api/demo-login`, off by default in prod). **Sprint 19 (Anton wave):** 20+ iOS icon sizes, screenshots per device class (iPhone 6.7"/6.1"/5.5", Android phone+tablet), legal pages reviewed by PH lawyer + hosted at akbai.com/privacy + /terms. |
| G6 | Kai character evolution (Gemini image generation) | IMPORTANT | Sprint 14 (iteration) + Sprint 18 (usage doc) → Sprint 19 (art swap) | 🟡 USAGE DOC + FALLBACK DOCUMENTED 2026-05-29 (Sprint 18): `kai-character-usage.md` maps Kai across 13 product surfaces (≥5-surface gate PASS) + defines the fallback policy when expression art is missing. Anton iterates Gemini prompts using `akbai-delivery/skills/ux-designer/references/kai-gemini-prompts.md` (₱0 budget; locked Character DNA preamble + reference-image input mitigates drift; 8 poses + 1 hero target). **Sprint 19 (Anton wave):** real art swap — replace static `kai-mark.png` usages with the finalized Gemini pose set. |
| G7 | Pre-Launch Feature Readiness Gate | **CRITICAL** | ✅ CODE-SIDE GREEN 2026-05-29 (Sprint 18) — public-release items Anton-gated Sprint 19 | Comprehensive 40+ item checklist (tier-defining features, payment/entitlements, brand/UX, tech health, compliance, distribution readiness) must be GREEN before Sprint 19 public release. **Sprint 18 drove all agent-doable §11 items to GREEN** and produced the traffic-light gate report at `akbai-delivery/shared/sprint-18-prelaunch-gate.md`. Shipped this sprint: Build 5 reconciliation (was claimed done Sprint 10 but never in `main` — built for real), trial-countdown banner + day-7 paywall, circuit-breaker→Sentry alert, push 7/3/1-day cron scheduler (`/api/cron/deadline-notifications` + `vercel.json` + `CRON_SECRET`), offline receipt scan queue, CSV export with OWASP formula-injection guard, tax-year 2027 rollover test (+ Manila-tz bug fix), reviewer demo-mode, legal/store/Kai DRAFTS, enye-rendering audit. Security audit: NO BLOCKERS (8 findings all fixed in-sprint). **1716 tests passing.** **Sprint 19 (Anton wave) closes the gate fully:** Apple/Google enrollment, real RevenueCat creds + sandbox SKUs + curl smoke, iOS icons/screenshots/legal hosting, real Kai art, on-device Pixel 5 smoke. Apps stay in TestFlight + Play Internal Testing until these public-release items land. Full checklist in plan §11. |
...
**Rule:** All CRITICAL gaps are hard gates. No public release proceeds until these are resolved. G7 (Pre-Launch Gate) is code-side GREEN as of Sprint 18; G5/G6 drafts/usage docs done. The remaining public-release work (Apple/Google enrollment, real RevenueCat creds, iOS assets + legal hosting, real Kai art, on-device smoke) is Anton-gated and closes G2/G4/G5/G6/G7 at the Sprint 19 launch wave.
