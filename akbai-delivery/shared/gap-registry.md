...
| # | Gap | Severity | When to Fix | Action |
|---|-----|----------|-------------|--------|
| B1 | AI loading states | IMPORTANT | Build 3 | ✅ RESOLVED 2026-03-26. `loading-estimator.ts` returns feature/tier-specific wait estimates. Message-list shows Taglish text below bouncing dots ("Nag-iisip si Kai... ~3-5 seconds"), switches to long-wait message after 5s. |
...
| D6 | Session expiry UX | IMPORTANT | Build 3 | ✅ RESOLVED 2026-03-26. `session-expiry-modal.tsx` with glassmorphism overlay and Taglish copy ("Oops, nag-expire ang session mo"). `useSessionWatcher()` hook detects unexpected sign-out via `onAuthStateChange()`. `SessionGuard` integrated into app layout. Preserves chat draft in localStorage. |
...
3. **Taglish Style Guide + Prompt Regression Tests** — ✅ RESOLVED 2026-03-26. 25-case prompt regression test suite in `prompt-regression.test.ts`: 5 groups (persona integrity, Taglish compliance, feature prompts, guardrails, integration). All deterministic — no API calls. Taglish style guide (357 lines, 12 sections) was already complete from Sprint 2.
4. **Kai Error Acknowledgement Pattern** — Pre-drafted Taglish response for when Kai surfaces incorrect data. Pattern: acknowledge clearly → take responsibility → explain what happened → offer concrete next step.
