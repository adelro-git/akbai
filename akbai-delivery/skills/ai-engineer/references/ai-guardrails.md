...
AKBai handles people's money and tax compliance. Users trust KA with their financial data and rely on it for BIR deadline tracking. A wrong number, a missed disclaimer, or a hallucinated amount can cost a user real money or create legal exposure.
...
AKBai uses two conversational disclaimers (appended to KA's output) and one persistent UI disclaimer:
...
This function runs on every KA output before it reaches the client. If the prompt already included the disclaimer (which it should), the dedup check prevents doubling.
...
KA never invents financial amounts. Every number in KA's output must trace back to either:
...
If KA needs a number it doesn't have, it asks for it. "Magkano ba ang ingredients mo per batch?" is always better than estimating.
...
If verification fails, KA shows a warning: "May discrepancy sa numbers — i-check natin ulit."

#### 3c. Explicit Uncertainty

When KA is uncertain, it says so. The system prompt includes:
...
BIR rules and deadlines are stored as reference data in Supabase (`bir_deadlines` table and a reference constants file), not generated by Claude. KA looks them up, not makes them up. If a user asks about a BIR rule that's not in the reference data, KA says: "Hindi ko pa alam ang sagot diyan — best if i-check mo sa BIR website or tanungin ang CPA mo."
...
The system prompt instructs KA to self-report uncertainty: "If you're not sure about something, say 'Hindi ko sure pero...' and explain your reasoning."
...
When the circuit breaker trips, KA responds warmly — never with a raw error:
...
2. KA immediately responds with the Trust Recovery Pattern (see SKILL.md)
3. The flag is queued for Anton's review in the admin dashboard (Gap D10)
4. If the flag is on a financial amount, KA offers manual correction immediately
...
|----------|-------|-------|
| KA voice | 5 | Taglish blend natural? "Po" appropriate? No corporate filler? |
...
| **P1 — High** | KA persona completely broken, prompt injection succeeded, disclaimer missing | Same day |
...
2. **Acknowledge**: KA sends a message to affected users: "May na-detect kaming issue — ini-investigate na namin."
