...
- `references/ui-ux-principles-akbai.md` — **Applied UX laws & retention principles**, distilled from a NotebookLM corpus (Laws of UX, NN/g, Don Norman, Hooked, WCAG, web.dev). Use as a design-review checklist. Includes the 10 principles that shape AKBai, accessibility/perf non-negotiables, onboarding rules, and the top-3 retention investments. **Load by default before any UI design or review work.**
- `references/conversational-filipino-manual.md` — **The authoritative conversational Filipino do's and don'ts manual.** Curated real-world examples of natural conversational Filipino vs awkward/wrong phrasing. This is the single source of truth for how Kai speaks — overrides conflicting guidance in other files. (Includes Phase 1 research additions in §6 for family-victory weekly-recap framing.)
- `references/conversational-filipino-copy-guide.md` — Kai voice rules, 10 do/don't examples, tone calibration by context, microcopy patterns. Companion to the manual.
- `references/filipino-text-vernacular.md` — How users type *to* Kai (text shortcuts, slang, business shorthand). Read before designing any input or chat interpretation logic.
- `references/design-system.md` — **The authoritative design system specification.** Creative north star ("Sun-Drenched Atelier"), color token system, elevation rules (No-Line Rule, Tonal Layering, Ambient Shadows), component guidelines, and (Phase 1 additions) Don Norman emotional layers + Decorative Motif Vocabulary (banig/capiz/sampaguita IN; bahay-kubo/fiesta/saint imagery OUT). Visual source of truth for how AKBai looks and feels.
- `references/mobile-first.md` — PWA constraints, touch targets, card layout system, offline behavior, camera integration. (Phase 1 addition: Filipino Connectivity & Device Baseline in §1 — 35 Mbps median, mid-range Android, prepaid-aware budgets.)
...
- `references/research-sources/README.md` + `RESEARCH-FINDINGS-MAP.md` — NotebookLM notebooks (UI/UX `0127f12f-…`, Filipino MSME `8ee05ad7-…`) for cross-phase re-querying, plus a one-page provenance map of where each Phase 1 research finding landed in the canonical docs.

Read the specific reference file that matches your task. If designing an onboarding screen, start with `ux-flows.md` + `ui-ux-principles-akbai.md`. If writing error messages or any user-facing copy, **always** start with `conversational-filipino-manual.md` (single source of truth for Kai's voice). If specifying layout or interactions, start with `mobile-first.md` + `ui-ux-principles-akbai.md`. For decorative SVG / motif decisions or warmth philosophy, see `design-system.md` §1 (Don Norman layers) and §6 (Decorative Motif Vocabulary). For any UI design review, `ui-ux-principles-akbai.md` is the default checklist.
